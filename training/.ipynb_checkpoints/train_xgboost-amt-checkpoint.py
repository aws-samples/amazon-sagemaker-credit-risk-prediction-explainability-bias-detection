...
def eval_multi_objective_metric(di_metric):
    #TODO - create a new metric that averages stastical_disparity with validation_auc
    multi_obj_metric = 1
    
    return "validation:auc", avg_metric
...
def main():
    ...
    params = {
        ...
        "silent": args.silent,
        "objective": objective_metric_name,
    ...
    bst = xgboost.train(params=params, dtrain=dtrain, evals=watchlist, num_boost_round=args.num_round)
    
    # Compute DI metric
    di_metric = statistical_disparity(bst, val_X, val_y, val_X[:, -1])
    
    #TODO: create a new metric that averages stastical_disparity with validation_auc
    mo_metric = eval_multi_objective_metric(di_metric)
    
    
    #TODO: How to update the objective name after the mdoel is training. Example was using "feval" before the training was done.
    # [0]#011validation-auc:{mo_metric}.
